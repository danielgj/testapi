...
const goblinDB = GDB(configParams.dbConfig, err => {
  if (err) {
  ...
  users = goblinDB.get('users');
  if (!users) {
    users = [];
  }
  sessions = goblinDB.get('sessions');
  if (!sessions) {
    sessions = [];
  }
  

  goblinDB.set(users, 'users');
  goblinDB.set(sessions, 'sessions');
